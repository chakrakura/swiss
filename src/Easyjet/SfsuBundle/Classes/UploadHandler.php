...
class UploadHandler
{
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        if ($post_max_size && ($content_length > $post_max_size)) {
            $file->error = $this->get_error_message('post_max_size') . " ($post_max_size)";
            return false;
        }
       
        if (!preg_match($this->options['accept_file_types'], $file->origFilename)) {
            $file->error = $this->get_error_message('accept_file_types');
            return false;
        }
        
              
        $mime_type = strtolower(mime_content_type($uploaded_file)); 
        if (!in_array ($mime_type, $this->options['accept_mime_types'])) {
            $file->error = $this->get_error_message('accept_file_types');
            return false;
        }
    ...
    protected function get_unique_filename($name, $type, $index, $content_range) {
        ...
        $uploaded_bytes = $this->fix_integer_overflow(intval(0));
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        do {
            $file->origFilename = $name;
            $file->name = "file-" . uniqid();
        } while(is_file($this->get_download_url($file->name)));
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
                ...
                if ($append_file) {
                    ...
                    try {
                        ...
                        if($this->uploadCallback()) {
                            ...
                            $file->fileId = $callback($file->name, $name);
                        }
            ...
            if ($file_size === $file->size) {
                $file->url = $this->get_download_url($file->name);
                list($img_width, $img_height) = @getimagesize($file_path);
                // not required any more as we are not displaying thumbnail,,
                /*if (is_int($img_width)) {
    ...
    /**
     * Set a function to launch after the file has been moved to the right folder
     * @param \Closure $callbackFunction
     * @return \Igestis\Utils\UploadHandler
     */
    public function setUploadedCallback(\Closure $callbackFunction) {
        $this->options['uploaded_callback'] = $callbackFunction;
        return $this;
    }
    
    /**
     * Return the callback function to launch after the upload is complete
     * @return \Closure|null
     */
    private function uploadCallback() {
        return $this->options['uploaded_callback'];
    }
}
