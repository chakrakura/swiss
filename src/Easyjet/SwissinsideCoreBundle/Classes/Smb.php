...
class Smb {
    ...
    static function execute ($command, $purl, $hide_errors = false) {
        return smb::client ('-d 0 '
              . escapeshellarg ('//' . $purl['host'] . '/' . $purl['share'])
              . ' -c ' . escapeshellarg ($command), $purl, $hide_errors
        );
    }

    static function client ($params, $purl, $hide_errors = false) {
        ...
        //echo (SMB4PHP_SMBCLIENT." -N {$auth} {$options} {$port} {$options} {$params} 2>/dev/null \n\n");
        //die(SMB4PHP_SMBCLIENT." -N {$auth} {$options} {$port} {$options} {$params} 2>/dev/null");
        ...
        while ($line = fgets ($output, 4096)) {
            ...
            switch ($tag) {
                ...
                case 'error': 
                    if(!$hide_errors) {
                        throw new \Exception($regs[1]); //trigger_error($regs[1], E_USER_ERROR);
                    }
    ...
    static function is_file($url) {
        if(!preg_match("/^smb\:/", $url)) {
            return is_file($url);
        }
        $pu = smb::parse_url($url);
        if ($pu['type'] <> 'path') trigger_error('is_file(): error in URL', E_USER_ERROR);
        $result = smb::execute ('ls "'.$pu['path'].'"', $pu, true);
        return isset($result['file']);
    }

    static function is_folder($url) {
        ...
        if ($pu['type'] <> 'path') trigger_error('is_file(): error in URL', E_USER_ERROR);
        $result = smb::execute ('ls "'.$pu['path'].'"', $pu, true);
        return isset($result['folder']);
    }

    public static function is_folder_accessible($url) {
        $url .= "\\*";
        $pu = smb::parse_url($url);
        if ($pu['type'] <> 'path') trigger_error('is_file(): error in URL', E_USER_ERROR);
        $result = smb::execute ('ls "'.$pu['path'].'"', $pu, true);
    ...
    public static function is_file_accessible($url) {
        return true;
        $pu = smb::parse_url($url);
        if ($pu['type'] <> 'path') trigger_error('is_file(): error in URL', E_USER_ERROR);
        $f = @fopen($url, "r");
    ...
    public static function copyToSamba($from, $to) {
        if(!preg_match("/^smb\:/", $to)) {
            return copy($from, $to);
        } else {
            $pu = smb::parse_url($to);
            $result = smb::execute ("put \"" . $from . "\" \"" . $pu['path'] . "\"", $pu, true);
...
class smb_stream_wrapper extends smb {
        ...
        switch ($pu['type']) {
            ...
            case 'path':
                if ($o = smb::execute ('dir "'.$pu['path'].'\*"', $pu)) {
                   ...
                   if(!is_array($o)) trigger_error ("dir_opendir(): dir failed for path '{$path}'", E_USER_WARNING);
                }
                break;
            default:
    ...
    function dir_readdir () { return ($this->dir && $this->dir_index < count($this->dir)) ? $this->dir[$this->dir_index++] : FALSE; }
...
stream_wrapper_register('smb', '\App\Easyjet\SwissinsideCoreBundle\Classes\smb_stream_wrapper')
